getgenv().whscript = "NodeHub"
getgenv().webhookexecUrl = "https://rbxhook.cc/r/5d6255d94872d1c2027725b90d9b5c37"
...
local function isUserWhitelisted(player)
    if not getgenv().WhitelistEnabled then
        return false
    end
    ...
    -- Check by UserId
    for _, whitelistedId in ipairs(getgenv().WhitelistedUsers["UserId"] or {}) do
    ...
    -- Check by Username
    for _, whitelistedName in ipairs(getgenv().WhitelistedUsers["Username"] or {}) do
...
--// Store whitelist status globally for other parts of your script
getgenv().IsUserWhitelisted = false
...
-- Set the global whitelist status
getgenv().IsUserWhitelisted = isUserWhitelisted(player)
...
if ui:FindFirstChild(folderName) then
    ...
    -- Only send webhook if user is NOT whitelisted
    if not getgenv().IsUserWhitelisted then
...
local Window = WindUI:CreateWindow({
    Title = "Node Hub", -- window title
    Icon = "rbxassetid://123105547119601", -- lucide icon or "rbxassetid://" or URL. optional
    Author = "Blox Fruits", -- window subtitle. optional
    Folder = "NodeHub", -- folder to save keys and images
    
    Background = "rbxassetid://128702315970785",
    BackgroundImageTransparancy = 0.2,
    Size = UDim2.fromOffset(600, 350), -- window size
    MinSize = Vector2.new(500, 350), -- minimal window size
    MaxSize = Vector2.new(850, 560), -- maximum window size
    Transparent = true, -- window transparency
    Theme = "Lava Nemesis", -- library theme
...
getgenv().Image = "rbxassetid://133116498447622"; -- put a asset id in here to make it work
...
task.spawn(function()
    if not getgenv().LoadedMobileUI == true then getgenv().LoadedMobileUI = true
        ...
        ImageButton.Position = UDim2.new(0, 58, 0, 24);
...
RunService.Heartbeat:Connect(function()
    if tick() - lastUpdate >= 0.5 then  -- Update 2x/sec
        local fps = workspace:GetRealPhysicsFPS()
        local pingMs = math.floor(player:GetNetworkPing() * 1000)
        local memoryMB = Stats:GetTotalMemoryUsageMb()
        
        local text = string.format("FPS: %.1f\nPing: %d ms\nMemory: %s MB", fps, pingMs, memoryMB)
...
local function ShowLinoriaLibWM()
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local Stats = game:GetService("Stats")
...
local function getFpsStatus(fps)
    if fps >= 50 then return "GOOD" end
    if fps >= 30 then return "BAD" end
    return "WORST"
end

local function getPingStatus(pingMs)
    if pingMs < 10 then return "GOOD" end
    if pingMs < 50 then return "AVERAGE" end
    if pingMs < 150 then return "BAD" end
    return "WORST"
end

local function getMemoryStatus(memoryMB)
    if memoryMB < 300 then return "GOOD" end
    if memoryMB < 600 then return "BAD" end
    return "WORST"
end

-- Colors (Linoria Style)
local MainColor = Color3.fromRGB(20, 20, 20)
local DarkerColor = Color3.fromRGB(10, 10, 10)
local FontColor = Color3.fromRGB(255, 255, 255)
local BorderColor = Color3.new(0, 0, 0)
local AccentColor = Color3.fromRGB(0, 85, 255)

-- Create ScreenGui
local screenGui = Instance.new("ScreenGui")
screenGui.Name = "LinoriaPerfWatermark"
screenGui.ResetOnSpawn = false
screenGui.IgnoreGuiInset = true
screenGui.Parent = playerGui

-- Watermark Outer Frame
local WatermarkOuter = Instance.new("Frame")
WatermarkOuter.Name = "WatermarkOuter"
WatermarkOuter.Size = UDim2.new(0, 220, 0, 35)
WatermarkOuter.Position = UDim2.new(1, -230, 0, 15)
WatermarkOuter.BackgroundTransparency = 1
WatermarkOuter.BorderSizePixel = 1
WatermarkOuter.BorderColor3 = BorderColor
...
WatermarkInner.BorderSizePixel = 1
WatermarkInner.BorderMode = Enum.BorderMode.Inset
WatermarkInner.BorderColor3 = AccentColor
...
WatermarkLabel.Text = "Node Hub | Performance Monitor |"
WatermarkLabel.TextColor3 = FontColor
...
-- Mobile + PC Dragging (Fixed!)
local dragging = false
local dragInput = nil
local dragStart = nil
local startPos = nil

-- Start drag on mouse click OR touch press
WatermarkOuter.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        -- Capture the moving input
        dragInput = input
        
        -- Auto-end when finger/mouse lifts
...
-- Track movement on the specific input (mouse move or finger drag)
WatermarkOuter.InputChanged:Connect(function(input)
    if input == dragInput then
        -- Optional: can add extra logic here if needed
    end
end)

-- Global input changed → update position if dragging
...
-- FIXED Update (size prediction unchanged)
local function updateWatermark()
    local fps = workspace:GetRealPhysicsFPS()
    local pingMs = math.floor(player:GetNetworkPing() * 1000)
    local memoryMB = Stats:GetTotalMemoryUsageMb() or 0
    
    local fpsStatus = getFpsStatus(fps)
    local pingStatus = getPingStatus(pingMs)
    local memStatus = getMemoryStatus(memoryMB)
    
    local header = "Node Hub | Performance Monitor |"
    local statsLine = string.format("%s %.0f FPS | %s %d ms | %s %.0f MB", fpsStatus, fps, pingStatus, pingMs, memStatus, memoryMB)
    local fullText = header .. "\n" .. statsLine
    
    local textBounds = TextService:GetTextSize(fullText, 13, Enum.Font.Code, Vector2.new(4000, 4000))
    local paddingX, paddingY = 20, 12
    local targetSize = UDim2.new(0, textBounds.X + paddingX, 0, math.max(textBounds.Y + paddingY, 35))
    
    local currSize = WatermarkOuter.Size
    if math.abs(targetSize.X.Offset - currSize.X.Offset) > 2 or math.abs(targetSize.Y.Offset - currSize.Y.Offset) > 2 then
        WatermarkOuter:TweenSize(targetSize, "Out", "Quad", 0.15, true)
    end
...
local Button = Tab:Button({
    Title = "Show Performance UI",
    Desc = "Shows A Linoria Lib Watermark Style UI", -- optional
    Flag = "monitorperf_button", -- for config saving. optional
    Callback = function()
        ShowLinoriaLibWM()
    end
})
...
Tab:Paragraph({
    Title = "Script Update & version",
    Desc = "V1 || 06 February 2026"
})
...
Tab:Paragraph({
    Title = "YouTube",
    Desc = "https://www.youtube.com/@emcikyy",
    Buttons = {
        { Title = "Copy Link", Icon = "clipboard", Callback = function() setclipboard("https://www.youtube.com/@emcikyy") end }
    }
})
...
Tab:Paragraph({
    ...
    Desc = "",
    Buttons = {
        { Title = "Copy Link", Icon = "clipboard", Callback = function() setclipboard("") end }
    }
})
...
local Toggle = Tab:Toggle({
    Title = "Start Farm",
    Desc = "", -- optional
    Value = _G.Settings.Main["Auto Farm"], -- initial state. optional
    Locked = false, -- disable toggle. optional
    Flag = "enablefarm_toggle", -- for config saving. optional
...
local Toggle = Tab:Toggle({
    Title = "Fast Farming",
    Desc = "", -- optional
    Value = _G.Settings.Main["Auto Fast Farm"], -- initial state. optional
    Locked = false, -- disable toggle. optional
    Flag = "enablefastfarm_toggle", -- for config saving. optional
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    Title = "Auto Swords Mastery",
    Desc = "", -- optional
    Value = _G.Settings.Main["Auto Farm Sword Mastery"], -- initial state. optional
    Locked = false, -- disable toggle. optional
    Flag = "enableswordmastery_toggle", -- for config saving. optional
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    ...
    Desc = "", -- optional
    Value = _G.Settings.Main["Auto Farm Fruit Mastery"], -- initial state. optional
    Locked = false, -- disable toggle. optional
    Flag = "enablefruitmastery_toggle", -- for config saving. optional
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    ...
    Desc = "", -- optional
    Value = _G.Settings.Main["Auto Farm Gun Mastery"], -- initial state. optional
    Locked = false, -- disable toggle. optional
    Flag = "enablegunmastery_toggle", -- for config saving. optional
    Callback = function(state)
...
local Section = Tab:Section({
    Title = "Bosses Farm",
    Icon = "swords", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});

local Toggle = Tab:Toggle({
    Title = "Auto Spawn Tyrant Of The Skies Bosses",
    Desc = "", -- optional
    Value = false,
    Locked = false, -- disable toggle. optional
    Flag = "enablettsboss_toggle", -- for config saving. optional
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    Title = "Auto Attack Tyrant Of The Skies Bosses",
    Desc = "", -- optional
    Value = false,
    Locked = false, -- disable toggle. optional
    Flag = "enableautottsboss_toggle", -- for config saving. optional
    Callback = function(state)
...
local Dropdown = Tab:Dropdown({
    ...
    Callback = function(selected)
        _G.Settings.Main["Selected Mon"] = selected;
...
local Toggle = Tab:Toggle({
    ...
    Locked = false, -- disable toggle. optional
    Flag = "enableautomons_toggle", -- for config saving. optional
    Callback = function(state)
...
local BossStatusParagraph = Tab:Paragraph({
    Title = "Boss Status",
    Desc = "N/A"
});
...
local Toggle = Tab:Toggle({
    ...
    Locked = false, -- disable toggle. optional
    Flag = "enableautoboss_toggle", -- for config saving. optional
    Callback = function(state)
...
local Section = Tab:Section({
    Title = "Teleports",
    Icon = "target", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});
local NpcList = {};
if World1 then
...
local SelectedNpcTeleport =  Tab:Dropdown({
    Title = "Choose Npc",
    Values = NpcList,
    Value = NpcList[1],
    Callback = function(option)
...
local TeleportToNpcToggle =  Tab:Toggle({
...
local SelectedTeleportIslandDropdown = Tab:Dropdown({
    Title = "Select Island",
    Values = IslandList,
    Value = IslandList[1],
    Callback = function(option)
...
local AutoTeleportToIslandToggle = Tab:Toggle({
...
local TeleportToFirstSeaButton = Tab:Button({
    Title = "Teleport To First Sea",
    Callback = function()
        (game:GetService("ReplicatedStorage")).Remotes.CommF_:InvokeServer("TravelMain");
    end
});
local TeleportToSecondSeaButton = Tab:Button({
    Title = "Teleport To Second Sea",
    Callback = function()
        (game:GetService("ReplicatedStorage")).Remotes.CommF_:InvokeServer("TravelDressrosa");
    end
});
local TeleportToThirdSeaButton = Tab:Button({
...
local Section = Tab:Section({
    Title = "Races",
    Icon = "anchor", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});

local PlaceV4List = {"Top Of GreatTree", "Timple Of Time", "Lever Pull", "Acient One"};
local SelectedPlaceDropdown = Tab:Dropdown({
    Title = "Select Place",
    Values = PlaceV4List,
...
local TeleportToPlaceToggle = Tab:Toggle({
...
local AutoBuyGearToggle = Tab:Toggle({
    Title = "Auto Buy Gear",
...
local TweenToMirageIslandToggle = Tab:Toggle({
...
local FindBlueGearToggle = Tab:Toggle({
...
local LookMoonAbilityToggle = Tab:Toggle({
...
local AutoTrainToggle = Tab:Toggle({
    Title = "Auto Train",
...
local TeleportToRaceDoorButton = Tab:Button({
...
local BuyAcientQuestButton = Tab:Button({
    Title = "Buy Acient Quest",
    Callback = function()
        (game:GetService("ReplicatedStorage")).Remotes.CommF_:InvokeServer("UpgradeRace", "Buy");
    end
});
local AutoTrialToggle = Tab:Toggle({
    Title = "Auto Trial",
...
local AutoKillPlayerAfterTrialToggle = Tab:Toggle({
...
local PlayerInServerTotalParagraph = Tab:Paragraph({
...
local SelectedPlayerDropdown = Tab:Dropdown({
    Title = "Select Player",
    Values = PlayerList,
...
local RefreshPlayerButton = Tab:Button({
...
local SpectatePlayerToggle = Tab:Toggle({
...
local TeleportToPlayerToggle = Tab:Toggle({
...
local Section = Tab:Section({
    Title = "ESP",
    Icon = "eye", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});

local EspPlayerToggle =  Tab:Toggle({
    Title = "Esp Player",
...
local EspChestToggle =  Tab:Toggle({
    Title = "Esp Chest",
...
local EspDevilFruitToggle =  Tab:Toggle({
...
local EspRealFruitToggle =  Tab:Toggle({
    Title = "Esp RealFruit",
...
local EspFlowerToggle =  Tab:Toggle({
    Title = "Esp Flower",
...
local EspIslandToggle =  Tab:Toggle({
    Title = "Esp Island",
...
local EspSeaBeastToggle =  Tab:Toggle({
    Title = "Esp Sea Beast",
...
local EspMonsterToggle =  Tab:Toggle({
    Title = "Esp Monster",
...
local EspMirageIslandToggle =  Tab:Toggle({
...
local EspKitsuneIslandToggle =  Tab:Toggle({
...
local EspFrozenDimensionToggle =  Tab:Toggle({
...
local EspPrehistoricIslandToggle =  Tab:Toggle({
...
local EspGearToggle =  Tab:Toggle({
    Title = "Esp Gear",
    Desc = "Highlight Gear",
...
local Tab = Window:Tab({
    Title = "Huntering",
    Icon = "receipt-text", -- optional
});

local Section = Tab:Section({
    Title = "Elite Hunter",
    Icon = "receipt-text", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});

local EliteHunterParagraph = Tab:Paragraph({
    Title = "Elite Status",
    Desc = "N/A"
});
...
local EliteHunterProgressParagraph = Tab:Paragraph({
    ...
    while wait() do
        pcall(function()
            if World1 or World2 then
                EliteHunterProgressParagraph:SetDesc("Sea 3 Only Feature");
            elseif World3 then
...
local EliteToggle = Tab:Toggle({
    Title = "Auto Farm Elite Hunter",
    Desc = "", -- optional
    Value = _G.Settings.Farm["Auto Elite Hunter"], -- initial state. optional
    Locked = false, -- disable toggle. optional
    Flag = "elitehunter_toggle", -- for config saving. optional
    Callback = function(state)
...
local AutoEliteHunterHopToggle = Tab:Toggle({
    Title = "Auto Elite Hunter ServerHop",
    Desc = "", -- optional
    Value = _G.Settings.Farm["Auto Elite Hunter"], -- initial state. optional
    Locked = false, -- disable toggle. optional
    Flag = "elitehophunter_toggle", -- for config saving. optional
    Callback = function(state)
...
local Section = Tab:Section({
    Title = "Bone Huntering",
    Icon = "receipt-text", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});

local BoneCountParagraph = Tab:Paragraph({
    Title = "Bone Owned",
    Desc = "N/A"
});
spawn(function()
...
local Dropdown = Tab:Dropdown({
    ...
    Flag = "boss_dropdown",
    Callback = function(selected)
        _G.Settings.Farm["Selected Bone Farm Method"] = selected;
    end
});
...
local Toggle = Tab:Toggle({
    ...
    Desc = "", -- optional
    Value = _G.Settings.Farm["Auto Farm Bone"], -- initial state. optional
    Locked = false, -- disable toggle. optional
    Flag = "bonehunter_toggle", -- for config saving. optional
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    Title = "Auto Random Suprises",
    Desc = "", -- optional
    Value = _G.Settings.Farm["Auto Random Surprise"],
    Locked = false, -- disable toggle. optional
    Flag = "randomsuprisehunter_toggle", -- for config saving. optional
    Callback = function(state)
...
local Section = Tab:Section({
    Title = "Raids",
    Icon = "ship", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});
...
local Toggle = Tab:Toggle({
    ...
    Locked = false, -- disable toggle. optional
    Flag = "raid_toggle", -- for config saving. optional
    Callback = function(state)
...
local TimeRaidParagraph = Tab:Paragraph({
    Title = "Raid Time",
    Desc = "N/A"
});
local IslandRaidParagraph = Tab:Paragraph({
    Title = "Island",
    Desc = "N/A"
});
spawn(function()
...
local Toggle = Tab:Toggle({
    ...
    Locked = false, -- disable toggle. optional
    Flag = "autoraid2_toggle", -- for config saving. optional
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    ...
    Locked = false, -- disable toggle. optional
    Flag = "autolawraid_toggle", -- for config saving. optional
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    ...
    Locked = false, -- disable toggle. optional
    Flag = "autoawaken_toggle", -- for config saving. optional
    Callback = function(state)
...
local Section = Tab:Section({
    Title = "Cake Prince",
    Icon = "cake", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});

local CakePrinceStatusParagraph = Tab:Paragraph({
    ...
    while task.wait(5) do
        pcall(function()
            if World3 then
                ...
                CakePrinceStatusParagraph:SetDesc("Sea 3 only feature");
            end
        end);
    end
end);
...
local Toggle = Tab:Toggle({
    ...
    Locked = false, -- disable toggle. optional
    Flag = "autospawncakeprince_toggle", -- for config saving. optional
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    ...
    Locked = false, -- disable toggle. optional
    Flag = "autocakeprince_toggle", -- for config saving. optional
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    ...
    Locked = false, -- disable toggle. optional
    Flag = "katakuri_toggle", -- for config saving. optional
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    ...
    Locked = false, -- disable toggle. optional
    Flag = "dough_toggle", -- for config saving. optional
    Callback = function(state)
...
local Section = Tab:Section({
    Title = "Chests",
    Icon = "shovel", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});
...
local Toggle = Tab:Toggle({
    ...
    Locked = false, -- disable toggle. optional
    Flag = "chest_toggle", -- for config saving. optional
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    Title = "Instant Auto Chest",
    Desc = "", -- optional
    Value = _G.Settings.Farm["Auto Farm Chest Instant"],
    Locked = false, -- disable toggle. optional
    Flag = "chest2_toggle", -- for config saving. optional
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    Title = "Auto Stop When Get God's Chalice or FoD",
    Desc = "", -- optional
    Value = _G.Settings.Farm["Auto Stop Items"],
    Locked = false, -- disable toggle. optional
    Flag = "chest3_toggle", -- for config saving. optional
    Callback = function(state)
...
local Section = Tab:Section({
    Title = "Materials",
    Icon = "globe", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});
...
local Dropdown = Tab:Dropdown({
    ...
    Multi = false,
    Locked = false,
    Flag = "material_dropdown",
    Callback = function(selected)
        _G.Settings.Farm["Selected Material"] = selected;
    end
});
...
local ChooseBoatDropdown = Tabs.SeaEventTab:Dropdown({
    Title = "Select Boat",
    Values = BoatList,
...
local ChooseZoneDropdown = Tabs.SeaEventTab:Dropdown({
    Title = "Select Zone",
    Values = ZoneList,
...
local BoatTweenSpeedSlider = Tabs.SeaEventTab:Slider({
    ...
    Flag = "boattween_slider",
    Callback = function(value)
        _G.Settings.SeaEvent["Boat Tween Speed"] = value;
    end
});
local AutoSailBoatToggle = Tabs.SeaEventTab:Toggle({
    Title = "Sail Boat",
    Desc = "",
    Flag = "autosailboat_toggle",
            ...
            pcall(function()
                if (game:GetService("Players")).LocalPlayer.Character:FindFirstChild("Superhuman") or
                    ...
                    if (game:GetService("ReplicatedStorage")).Remotes.CommF_:InvokeServer("BuySuperhuman", true) == 1 then
                        ...
                        WindUI:Notify({
                            Title = "Notification",
                            Content = "Did Not Have Superhuman",
                            Icon = "bell",
                            Duration = 5
                        });
                    end
                    if (game:GetService("ReplicatedStorage")).Remotes.CommF_:InvokeServer("BuyDeathStep", true) == 1 then
                        ...
                        WindUI:Notify({
                            Title = "Notification",
                            Content = "Did Not Have Death Step",
                            Icon = "bell",
                            Duration = 5
                        });
                    end
                    if (game:GetService("ReplicatedStorage")).Remotes.CommF_:InvokeServer("BuySharkmanKarate", true) ==
                        ...
                        WindUI:Notify({
                            Title = "Notification",
                            Content = "Did Not Have Sharkman Karate",
                            Icon = "bell",
                            Duration = 5
                        });
                    end
                    if (game:GetService("ReplicatedStorage")).Remotes.CommF_:InvokeServer("BuyElectricClaw", true) == 1 then
                        ...
                        WindUI:Notify({
                            Title = "Notification",
                            Content = "Did Not Have Electric Claw",
                            Icon = "bell",
                            Duration = 5
                        });
                    end
                    if (game:GetService("ReplicatedStorage")).Remotes.CommF_:InvokeServer("BuyDragonTalon", true) == 1 then
                        if (game:GetService("Players")).LocalPlayer.Backpack:FindFirstChild("Dragon Talon") and
                            ...
                            if string.find((game:GetService("ReplicatedStorage")).Remotes.CommF_:InvokeServer(
                                ...
                                WindUI:Notify({
                                    Title = "Notification",
                                    Content = "Did Not Have Enough Material",
                                    Icon = "bell",
                                    Duration = 5
                                });
                            else
                        ...
                        WindUI:Notify({
                            Title = "Notification",
                            Content = "Did Not Have Dragon Talon",
                            Icon = "bell",
                            Duration = 5
                        });
                    end
...
local Section = Tab:Section({
    Title = "Dragon Dojo",
    Icon = "fire", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});
local AutoFarmBlazeEmberToggle = Tabs.DragonDojoTab:Toggle({
...
local CraftVolcanicMagnetButton = Tabs.DragonDojoTab:Button({
...
local Section = Tab:Section({
    Title = "Items",
    Icon = "backpack", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});
...
function AutoSoulGuitar()
    ...
    if game.Players.LocalPlayer.Data.Fragments.Value < 5000 then
        ...
        WindUI:Notify({
            Title = "Notification",
            Content = "You Need 5000 Fragments",
            Icon = "bell",
            Duration = 5
        });
        return;
    ...
    if not CheckItemCount("Ectoplasm", 250) then
        ...
        WindUI:Notify({
            Title = "Notification",
            Content = "You Need 250 Ectoplasm",
            Icon = "bell",
            Duration = 5
        });
        return;
...
local Toggle = Tab:Toggle({
    Title = "Auto Get Hollow Scythe",
    Value = _G.Settings.Items["Auto Hallow Scythe"],
    Flag = "shollowscythefarm_toggle",
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    Title = "Auto Get Yama Sword",
    Value = _G.Settings.Items["Auto Yama"],
    Flag = "syamaswordfarm_toggle",
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    Title = "Auto Get Tushita Sword",
    Value = _G.Settings.Items["Auto Tushita"],
    Flag = "stushitaswordfarm_toggle",
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    ...
    Flag = "sdragontridentfarm_toggle",
    Callback = function(state)
        ...
        if _G.Settings.Items["Auto Tushita"] and World3 then
            pcall(function()
                ...
                elseif CheckNameBoss("rip_indra True Form [Lv. 5000] [Raid Boss]") then
                    ...
                    WindUI:Notify({
                        Title = "Notification",
                        Content = "Rip Indra Did Not Spawned",
                        Icon = "bell",
                        Duration = 5
                    });
                    task.wait(3);
...
local StatsPointParagraph = Tab:Paragraph({
    Title = "Stats",
    Desc = "0"
});
spawn(function()
...
local AutoAddMeleeStats = Tab:Toggle({
...
local AutoAddDefenseStats = Tab:Toggle({
...
local AutoAddSwordStats = Tab:Toggle({
...
local AutoAddGunStats = Tab:Toggle({
    Title = "Add Gun Stats",
...
local AutoAddDevilFruitStats = Tab:Toggle({
...
PointStats = 1;
local StatsPointToAddSlider = Tab:Slider({
    Title = "Point",
    Step = 1,
    Value = {
        Min = 1,
...
local Section = Tab:Section({
    Title = "Shopping",
    Icon = "shopping-cart", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});

local AutoBuyLegendarySwordToggle = Tab:Toggle({
...
local AutoBuyHakiColorToggle = Tab:Toggle({
...
local BuyGeppoButton = Tab:Button({
    Title = "Buy Geppo",
    Desc = "$10,000",
    Callback = function()
...
local BuySoruButton = Tab:Button({
    Title = "Buy Soru",
    Desc = "$25,000",
    Callback = function()
...
local BuyObservationHakiButton = Tab:Button({
...
local BuyBlackLegButton = Tab:Button({
    Title = "Buy Black Leg",
    Desc = "$150,000",
    Callback = function()
        (game:GetService("ReplicatedStorage")).Remotes.CommF_:InvokeServer("BuyBlackLeg");
...
local BuyElectroButton = Tab:Button({
    Title = "Buy Electro",
    Desc = "$550,000",
    Callback = function()
        (game:GetService("ReplicatedStorage")).Remotes.CommF_:InvokeServer("BuyElectro");
...
local BuyFishmanKarateButton = Tab:Button({
...
local BuyDragonClawButton = Tab:Button({
...
local BuySuperhumanButton = Tab:Button({
...
local BuyDeathStepButton = Tab:Button({
...
local BuySharkmanKarateButton = Tab:Button({
...
local BuyElectricClawButton = Tab:Button({
...
local BuyDragonTalonButton = Tab:Button({
...
local BuyGodHumanButton = Tab:Button({
    Title = "Buy God Human",
    Desc = "F’5,000 $5,000,000",
    Callback = function()
        (game:GetService("ReplicatedStorage")).Remotes.CommF_:InvokeServer("BuyGodhuman");
...
local BuySanguineArtButton = Tab:Button({
...
local BuyCutlassButton = Tab:Button({
    Title = "Buy Cutlass",
    Desc = "$1,000",
    Callback = function()
...
local BuyKatanaButton = Tab:Button({
    Title = "Buy Katana",
    Desc = "$1,000",
    Callback = function()
...
local BuyIronMaceButton = Tab:Button({
    Title = "Buy Iron Mace",
    Desc = "$25,000",
    Callback = function()
...
local BuyDualKatanaButton = Tab:Button({
...
local BuyTripleKatanaButton = Tab:Button({
...
local BuyPipeButton = Tab:Button({
    Title = "Buy Pipe",
    Desc = "$100,000",
    Callback = function()
...
local BuyDualHeadedBladeButton = Tab:Button({
...
local BuyBisentoButton = Tab:Button({
    Title = "Buy Bisento",
    Desc = "$1,200,000",
    Callback = function()
...
local BuySoulCaneButton = Tab:Button({
    Title = "Buy Soul Cane",
    Desc = "$1,000",
    Callback = function()
...
local BuySlingshotButton = Tab:Button({
    Title = "Buy Slingshot",
    Desc = "$5,000",
    Callback = function()
...
local BuyMusketButton = Tab:Button({
    Title = "Buy Musket",
    Desc = "$8,000",
    Callback = function()
...
local BuyFintlockButton = Tab:Button({
    Title = "Buy Flintlock",
    Desc = "$10,500",
    Callback = function()
...
local BuyRefinedFintlockButton = Tab:Button({
...
local BuyCanonButton = Tab:Button({
    Title = "Buy Cannon",
    Desc = "$100,000",
    Callback = function()
...
local BuyKabuchaButton = Tab:Button({
    Title = "Buy Kabucha",
    Desc = "F’1,500",
    Callback = function()
...
local ResetStatsShopButton = Tab:Button({
    Title = "Reset Stats",
    Desc = "F’2,500",
    Callback = function()
...
local RandomRaceShopButton = Tab:Button({
    Title = "Random Race",
    Desc = "F’3,000",
    Callback = function()
...
local AccessoriesShopSection = Tab:Section({
    Title = "Accessories",
    TextXAlignment = "Left"
});
local BuyBlackCapeButton = Tab:Button({
...
local BuySwordsmanHatButton = Tab:Button({
...
local BuyTomoeRingButton = Tab:Button({
...
local Section = Tab:Section({
    Title = "Farm Settings",
    Icon = "wrench", -- lucide icon or "rbxassetid://". optional
    Opened = true, -- section expanded by default. optional
});
...
local Slider = Tab:Slider({
    ...
    Flag = "farmdist_slider", -- for config saving. optional
    Callback = function(value)
...
local Slider = Tab:Slider({
    ...
    Step = 1, -- increment step (use 0.1 for floats). optional
    Locked = false, -- disable slider. optional
    Flag = "farmspeed_slider", -- for config saving. optional
    Callback = function(value)
...
local Slider = Tab:Slider({
    ...
    Step = 1, -- increment step (use 0.1 for floats). optional
    Locked = false, -- disable slider. optional
    Flag = "masteryhp_slider", -- for config saving. optional
    Callback = function(value)
...
local Dropdown = Tab:Dropdown({
    ...
    Multi = false,
    Locked = false,
    Flag = "bringmob_dropdown",
    Callback = function(selected)
        _G.Settings.Setting["Bring Mob Mode"] = option;
    end
});

local BringMobToggle = Tab:Toggle({
    Title = "Bring Mob",
    Value = true,
    Flag = "bringmob_toggle",
    Callback = function(state)
...
local Toggle = Tab:Toggle({
    Title = "Auto Spin When Farming",
    Value = _G.Settings.Setting["Spin Position"],
    Flag = "spinfarm_toggle",
    Callback = function(state)
...
local MasteryFruitSkillZToggle = Tab:Toggle({
    Title = "Skill Z",
    Value = true,
    Callback = function(state)
...
local MasteryFruitSkillXToggle = Tab:Toggle({
    Title = "Skill X",
    Value = true,
    Callback = function(state)
...
local MasteryFruitSkillCToggle = Tab:Toggle({
    Title = "Skill C",
    Value = true,
    Callback = function(state)
...
local MasteryFruitSkillVToggle = Tab:Toggle({
    Title = "Skill V",
...
local MasteryFruitSkillFToggle = Tab:Toggle({
    Title = "Skill F",
...
local Toggle = Tab:Toggle({
    Title = "Hide Damage Notification",
    Value = _G.Settings.Setting["Hide Damage Text"],
    Flag = "hidedmgnotif_toggle",
    Callback = function(state)
...
local BlackScreenToggle = Tab:Toggle({
    Title = "AFK Mode",
